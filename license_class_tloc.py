...
class Tier_Allocation():

    def Tier_Allocation(AggMbps):
        if AggMbps <= 50:
            return "T0"
        elif 50 < AggMbps <= 400:
            return "T1"
...
class findTlocExt():
    ...
    def findIfTlocext(vmanage_host,vmanage_port,header,report_data,site_id,systemIPlst):

        getARP_edge1 = getData.getARP(vmanage_host,vmanage_port,header,systemIPlst[0])
        getARP_edge2 = getData.getARP(vmanage_host,vmanage_port,header,systemIPlst[1])
            ...
            if ((iter_edge1_ARP["vpn-id"] == "Default") or (iter_edge1_ARP["vpn-id"] == "0")) and (iter_edge1_ARP["mode"] == "ios-arp-mode-dynamic"):
                for iter_edge2_ARP in getARP_edge2:
                    if (((((iter_edge2_ARP["vpn-id"] == "Default") or (iter_edge2_ARP["vpn-id"] == "0")) and (iter_edge2_ARP["mode"] == "ios-arp-mode-interface")) and iter_edge1_ARP["hardware"] == iter_edge2_ARP["hardware"] ) and iter_edge1_ARP["address"] == iter_edge2_ARP["address"]):
                        if (re.split(r"\.", iter_edge2_ARP['interface'])[0]) in report_data[(str(site_id),iter_edge2_ARP["vdevice-name"])]["wanIFName-stats"] and (((re.split(r"\.", iter_edge2_ARP['interface'])[0]) not in report_data[(str(site_id),iter_edge2_ARP["vdevice-name"])]["TlocEXT-IfName"])):
                            report_data[(str(site_id),iter_edge2_ARP["vdevice-name"])]["TlocEXT-IfName"].append(re.split(r"\.", iter_edge2_ARP['interface'])[0])
            ...
            if ((iter_edge2_ARP["vpn-id"] == "Default") or (iter_edge2_ARP["vpn-id"] == "0")) and (iter_edge2_ARP["mode"] == "ios-arp-mode-dynamic"):
                for iter_edge1_ARP in getARP_edge1:
                    if (((((iter_edge1_ARP["vpn-id"] == "Default") or (iter_edge1_ARP["vpn-id"] == "0")) and (iter_edge1_ARP["mode"] == "ios-arp-mode-interface")) and iter_edge2_ARP["hardware"] == iter_edge1_ARP["hardware"] ) and iter_edge2_ARP["address"] == iter_edge1_ARP["address"]):
                        if ((re.split(r"\.", iter_edge1_ARP['interface'])[0]) in report_data[(str(site_id),iter_edge1_ARP["vdevice-name"])]["wanIFName-stats"]) and ((re.split(r"\.", iter_edge1_ARP['interface'])[0]) not in report_data[(str(site_id),iter_edge1_ARP["vdevice-name"])]["TlocEXT-IfName"]):
                            report_data[(str(site_id),iter_edge1_ARP["vdevice-name"])]["TlocEXT-IfName"].append(re.split(r"\.", iter_edge1_ARP['interface'])[0])
